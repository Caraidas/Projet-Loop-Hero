...
public class Player extends AbstractEntity implements Serializable {
	...
	private final HashMap<Ressource, Integer> ressources;
	 ...
	 */	
	private Player(int position, HashMap<Ressource, Integer> ressources, Inventory inventory, Deck deck, ArrayList<Item> items, HashMap<String, Double> basicStats, Range damage) {
	...
	public void addRessource(Ressource r, int n) { // adds n resources in the players "bag"
		
		if (ressources.containsKey(r)) {
			if (ressources.get(r) + n >= r.quantityToNext() && r.quantityToNext() != -1) {
				ressources.put(r.next(), 1);
				int difference = (ressources.get(r) + n) - r.quantityToNext();
				ressources.put(r, difference);
			} else {
				ressources.replace(r, ressources.get(r) + n);
			}
		} else {
			ressources.put(r, n);
		}	
	...
	public HashMap<Ressource, Integer> ressources() { // get the resources of the player
		return ressources;
	}
